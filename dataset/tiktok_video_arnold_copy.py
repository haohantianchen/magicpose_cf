            ...
            if self.train:
                try:
                    ...
                    if len(subject_folder_images) == 0 or len(subject_folder_images) == 1 or len(subject_pose_maps) == 0 or len(subject_pose_maps) == 1: # empty folder
                        print("empty source folder or folder with only one image detected")
                        continue
                    rand_int = torch.randint(low=1,high=len(subject_folder_images),size=(1,))
                    # 固定第一帧
                    condition_int = 0
                    input_int  = rand_int[0].item()
                    input_img_random_num = input_int % len(subject_folder_images)
                    input_pose_random_num = input_int % len(subject_pose_maps)
                    condition_img_random_num = condition_int % len(subject_folder_images)
                    condition_pose_random_num = condition_int % len(subject_pose_maps)
                    ...
                    # 根据subject_folder获得对应的prompts
                    prompts_df = pd.read_csv("/raid/lurenjie/AnimateDiff/dataset/tiktok_web10m/tiktok_web10m.csv")
                    prompts = prompts_df[prompts_df['videoid'] == int(subject_folder)]['name'].values[0]
                    res["video_id"] = subject_folder
                    res["prompts"] = prompts
                    yield res
...
def tiktok_video_arnold(**kwargs):
    train_data_path = "/raid/lurenjie/MagicDance/TikTok-v4/TikTok-v4-densepose_512_padd_resize/train_set"
    train_pose_path = "/raid/lurenjie/MagicDance/TikTok-v4/TikTok-v4-densepose_512_padd_resize/pose_map_train_set"
    return init_ds(train_data_path, train_pose_path, with_pose=True, **kwargs)


def tiktok_video_arnold_val(**kwargs):
    data_path = "/raid/lurenjie/MagicDance/TikTok-v4/TikTok-v4-densepose_512_padd_resize/val_set"
    pose_path = "/raid/lurenjie/MagicDance/TikTok-v4/TikTok-v4-densepose_512_padd_resize/pose_map_val_set"
