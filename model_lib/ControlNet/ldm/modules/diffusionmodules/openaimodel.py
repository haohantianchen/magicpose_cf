...
from model_lib.ControlNet.ldm.modules.attention import SpatialTransformer,SpatialTransformerCF
            ...
            if isinstance(layer, TimestepBlock):
                ...
                if uc:
                    ...
                    if attention_mode == 'read':
                        attn_index+=1    
            elif isinstance(layer, SpatialTransformerCF):
                # print("layer SpatialTransformerCF")
                if uc:
                    x = layer(x, context,uc=uc)
                else:
                    # pdb.set_trace()
    ...
    def __init__(
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            AttentionBlock(
                                ...
                                num_head_channels=dim_head,
                                use_new_attention_order=use_new_attention_order,
                            ) if not use_spatial_transformer else SpatialTransformerCF(
                                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                                disable_self_attn=disabled_sa, use_linear=use_linear_in_transformer,
                                use_checkpoint=use_checkpoint
                            )
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                num_head_channels=dim_head,
                use_new_attention_order=use_new_attention_order,
            ) if not use_spatial_transformer else SpatialTransformerCF(  # always uses a self-attn
                            ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                            disable_self_attn=disable_middle_self_attn, use_linear=use_linear_in_transformer,
                            use_checkpoint=use_checkpoint
                        ),
